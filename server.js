...
function parseScoresheet(html) {
  ...
  for (let i = 0; i < Math.min(lines.length, 30); i++) {
    if (lines[i].toUpperCase() === lines[i] && lines[i].length > 10 && /CHALLENGE|LEAGUE|CUP|TOURNAMENT|TIER|WOMEN|MEN|OPEN/.test(lines[i])) {
      title = lines[i]; break;
    }
  }
  ...
  const matches = [];
  let round = null, court = null;
  let i = 0;

  const isScore = v => { const n = parseInt(v); return !isNaN(n) && n >= 0 && n <= 30 && String(v).trim() === String(n); };
  const isName  = v => v.length > 0 && !isScore(v) && v !== 'Round' && !/^Court\s+\d+$/.test(v) && v !== 'Powered by Reclub' && v !== 'Print' && !/^Printable/.test(v) && !/^Note:/.test(v) && !/^This is/.test(v);
  ...
  while (i < lines.length) {
    ...
    if (line === 'Round') {
      ...
      if (next && !isNaN(parseInt(next))) { round = parseInt(next); i += 2; continue; }
    }

    if (/^Court\s+\d+$/.test(line)) { court = line; i++; continue; }

    if (round && court && i + 5 < lines.length) {
      const t1a = lines[i], t1b = lines[i+1], s1 = lines[i+2];
      const t2a = lines[i+3], t2b = lines[i+4], s2 = lines[i+5];
